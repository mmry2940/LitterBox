...
class EnhancedAdbManager {
  ...
  void _initializeClients() {
    ...
    _nativeClient!.connectionState.listen((state) {
      if (_currentBackend == AdbBackendType.native || _preferNativeForConnections) {
        _updateConnectionState(state);
  ...
  void setPreferNativeForConnections(bool prefer) {
    ...
    _addOutput('Native connections preference: ${prefer ? 'enabled' : 'disabled'}');
  }
    ...
    switch (_currentBackend) {
      ...
      case AdbBackendType.hybrid:
        if (_preferNativeForConnections) {
          ...
          _addOutput('Native connection failed, falling back to traditional ADB');
        }
  ...
  Future<bool> connectUsb(String deviceId) async {
    ...
    switch (_currentBackend) {
      case AdbBackendType.native:
        _addOutput('USB connections not supported in native mode, using traditional ADB');
        return await _traditionalClient!.connectUsb(deviceId);
  ...
  Future<bool> startLogcat([String filter = '']) async {
    ...
    if (activeClient == _nativeClient) {
      ...
      if (filter.isNotEmpty) {
        return await _traditionalClient!.startLogcat(logcatFilter: filter);
      } else {
  ...
  Future<bool> writeToShell(String input) async {
    ...
    if (activeClient == _nativeClient) {
      ...
      return await _traditionalClient!.writeToInteractiveShell(input);
    }
  }
  ...
  Future<String> pushFile(String localPath, String remotePath) async {
    if (_traditionalClient != null) {
      await _traditionalClient!.executeCommand('push "$localPath" "$remotePath"');
  ...
  Future<String> pullFile(String remotePath, String localPath) async {
    if (_traditionalClient != null) {
      await _traditionalClient!.executeCommand('pull "$remotePath" "$localPath"');
  ...
  dynamic _getActiveClient() {
    if (_state == ADBConnectionState.connected) {
      ...
      } else if (_traditionalClient?.currentState == ADBConnectionState.connected) {
  ...
  ADBOutputMode get outputMode => _traditionalClient?.outputMode ?? ADBOutputMode.raw;
  void setOutputMode(ADBOutputMode mode) => _traditionalClient?.setOutputMode(mode);
  
  List<String> get outputBuffer => _traditionalClient?.outputBuffer ?? [];
  void clearOutput() => _traditionalClient?.clearOutput();
  
  List<String> get commandHistoryList => _traditionalClient?.commandHistoryList ?? [];
  void clearHistory() => _traditionalClient?.clearHistory();
  
  Map<String, List<String>> get quickCommands => _traditionalClient?.quickCommands ?? {};
}
